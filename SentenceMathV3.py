...
class SentenceMath(nn.Module):
    ...
    def forward(self, input_ch1, mask1, input_ch2, mask2, word_ch_a, word_ch_b):
        ...
        sub = (rnn_out1 - rnn_out2) * (rnn_out1 - rnn_out2)

        # cos = torch.sum(rnn_out1 * rnn_out2, dim=1) / ((rnn_out1.norm(dim=1) * (rnn_out2.norm(dim=1))))
        # cos = cos.reshape(-1, 1)

        # out = self.cqattn(rnn_out1, rnn_out2, mask1, mask2)
        # out = out.transpose(1, 2).max(2)[0]
        return self.out2tag_logsoftmax(out - sub )
